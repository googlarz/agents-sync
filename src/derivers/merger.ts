 ...
 */
export function isManagedByAgentsSync(content: string): boolean {
 ...
 *   - the file is empty
 */
export async function loadUnmanagedFileAsCustomBlock(filePath: string): Promise<string[]> {
  const content = await readFileSafe(filePath);
  if (!content || content.trim().length === 0) return [];
  if (isManagedByAgentsSync(content)) return [];
